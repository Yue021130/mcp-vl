...
import { InternS1Service } from '../src/services/intern-s1-service';
import { AutoImageService } from '../src/services/auto-image-service';
import { config } from '../src/config/index';

const MODEL_TYPE = 'intern-s1' as const;
const MODEL_NAME = 'Intern-S1';

async function testConnection() {
  console.log(`=== 测试 ${MODEL_NAME} API 连接 ===`);

  const service = new InternS1Service();
...
function checkConfig(): boolean {
  ...
  if (!config.internS1.apiKey) {
    console.error(`❌ INTERNS1_API_KEY 未配置`);
    return false;
  }
...
async function main() {
  ...
  if (imagePath) {
    ...
    console.log('\nℹ️  未提供图片路径，跳过图片分析测试');
    console.log(`用法: npx ts-node scripts/test-interns1.ts <图片路径>`);
  }
