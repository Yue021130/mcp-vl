...
function checkConfig(): boolean {
  ...
  if (!config.qwenVL.apiKey) {
    console.error(`❌ QWENVL_API_KEY 未配置`);
    return false;
  }
...
async function main() {
  ...
  if (imagePath) {
    ...
    console.log('\nℹ️  未提供图片路径，跳过图片分析测试');
    console.log(`用法: npx ts-node scripts/test-qwen.ts <图片路径>`);
  }
